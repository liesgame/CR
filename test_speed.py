...
openai.base_url = 'http://59.77.5.88:7999/v1'
...
def get_completion_with_img_r1(thumbnail_path, system_prompt, prompt, processor, model=MODEL_NAME):
    client = OpenAI(api_key=openai.api_key,
                    base_url=openai.base_url
                    )
    messages=[
...
def get_completion_with_img_stream_r1(thumbnail_path, system_prompt, prompt, processor,  model=MODEL_NAME,):
    client = OpenAI(api_key=openai.api_key,
                    base_url=openai.base_url
                    )
    messages=[
    ...
    print(f"API call completion_tokens usage: {completion_tokens}")
            # usage.prompt_tokens / usage.completion_tokens / usage.total_tokens
    # 流式传输完成后，按照原来的逻辑分割内容
    thinking_part = reasoning_content.strip()
    answer_part = full_content.strip()

    print(f"API answer part: {answer_part}")
...
def get_completion_with_img_no_think(thumbnail_path, system_prompt, prompt, processor,  model=MODEL_NAME,):
    client = OpenAI(api_key=openai.api_key,
                    base_url=openai.base_url
                    )
    messages=[
    ...
    token = response.choices[0].message.content.split('</think>')
...
with open('/data/home/zhangchen/project/RL/SlideReasoner/datasets/Clv1_Vev2_Sev2_ReMuv1/summary/summary.json', 'r', encoding='utf-8') as f:
...
thumbnail_root = "/data/home/zhangchen/project/RL/SlideReasoner/datasets/resized_image_qwen3vl/mpp_10/image"
length_index = 0
Clinical_v1_Verify_v2_Selcet_v2_humen_ReRe_v2 = {}

script_start_time = time.time()
checkpoint_path     = '/data/home/zhangchen/models/Qwen3-VL-32B-Thinking'
...
while True:
    for i in Clinical_v1_Verify_v2_Selcet_v2_humen_ReMu_v1:
        ...
        exits_list = os.listdir('/data/home/zhangchen/project/RL/SlideReasoner/datasets/Clv1_Vev2_Sev2_ReRev3_32B_MPP10')
        ...
        while not successful and attempts < MAX_RETRIES:
            ...
            try:
                # 1. 呼叫 API
                reasoning, content, completion_tokens, prompt_tokens, total_tokens = get_completion_with_img_no_think(
